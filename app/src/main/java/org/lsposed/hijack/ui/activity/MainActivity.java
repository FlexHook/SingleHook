...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Arrays.stream(AppUtil.image.class.getFields()).forEach(field ->{
            try {
                Object value = field.get(null);
                if (value instanceof String) {
                    String link = (String) value;
                    if(!TextUtils.isEmpty(link)) Glide.with(getApplicationContext()).load(link).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).preload();
                }
            } catch (Throwable e) {}
        });
        // 设置状态栏
